                    ...
                    <div class="card-body">
                        <div class="tab-content" id="dashboardNavContent">
                            ...
                            <div class="tab-pane fade active show" id="activities" role="tabpanel" aria-labelledby="activities-pill">
                                ...
                                // Funzione per l'aggiornamento del creatore
                                function updateCreator($db, $id, $email, $password)
                                {
                                    $sql = "UPDATE dr_creator SET email_creator=?, password_creator=? WHERE id_creator=?";
                                    $stmt = $db->prepare($sql);
                                    $stmt->execute([$email, $password, $id]);
                                }

                                // Funzione per l'eliminazione del creatore
                                function deleteCreator($db, $id)
                                {
                                    $sql = "DELETE FROM dr_creator WHERE id_creator=?";
                                    $stmt = $db->prepare($sql);
                                    $stmt->execute([$id]);
                                }

                                // Verifica se è stata inviata una richiesta per aggiornare i dati
                                if (isset($_POST['update_creator'])) {
                                    $id = $_POST['id'];
                                    $email = $_POST['email'];
                                    $password = $_POST['password'];

                                    updateCreator($db, $id, $email, $password);
                                }

                                // Verifica se è stata inviata una richiesta per eliminare il creatore
                                if (isset($_POST['delete_creator'])) {
                                    $id = $_POST['id'];

                                    deleteCreator($db, $id);
                                }
                                // Query per ottenere i dati dei creatori dalla tabella 'dr_creator'
                                $sql = "SELECT * FROM dr_creator";
                                $result = $db->query($sql);

                                // Verifica se sono presenti risultati
                                if ($result->rowCount() > 0) {
                                    // Output dei dati dei creatori in una tabella
                                    ...
                                    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                                        ...
                                        echo "<td>" . $row["password_creator"] . "</td>";
                                        // Form per l'aggiornamento dei dati
                                        echo "<td>";
                                        echo "<details>";
                                        echo "<summary>Update</summary>";
                                        echo "<form method='post' action=''>";
                                        echo "<input type='hidden' name='id' value='" . $row["id_creator"] . "'>";
                                        echo "<input type='email' name='email' value='" . $row["email_creator"] . "' required>";
                                        echo "<input type='password' name='password' value='" . $row["password_creator"] . "' required>";
                                        echo "<input type='submit' name='update_creator' value='Update' class='btn btn-info'>";
                                        echo "</form>";
                                        echo "</details>";
                                        echo "</td>";


                                        // Form per l'eliminazione del creatore
                                        echo "<td>";
                                        echo "<form method='post' action=''>";
                                        echo "<input type='hidden' name='id' value='" . $row["id_creator"] . "'>";
                                        echo "<input type='submit' name='delete_creator' value='Delete' class='btn btn-danger' onclick='return confirm(\"Sei sicuro di voler eliminare questo creatore?\")'>"; // Aggiungi la classe btn-danger qui
                                        echo "</form>";
                                        echo "</td>";
                                    }
                                    echo "</tbody>";
                                    ...
                                    echo "<p>Nessun creatore trovato nel database.</p>";
                                }
                                ?>

                                <a href="creation.php" class="btn btn-success">
