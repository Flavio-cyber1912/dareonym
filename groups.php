                ...
                <div class="card mb-4">
                    <div class="card-header border-bottom">
                        ...
                        <ul class="nav nav-tabs card-header-tabs" id="dashboardNav" role="tablist">
                            <li class="nav-item"><a class="nav-link active" id="activities-pill" href="#activities" data-bs-toggle="tab" role="tab" aria-controls="activities" aria-selected="true">Groups</a></li>
                        </ul>
                    </div>
                    <div class="card-body">
                        <div class="tab-content" id="dashboardNavContent">
                            ...
                            <div class="tab-pane fade active show" id="activities" role="tabpanel" aria-labelledby="activities-pill">
                                ...
                                function updateGroup($db, $id, $name, $color)
                                {
                                    $sql = "UPDATE dr_group SET name_group=?, color_group=? WHERE id_group=?";
                                    $stmt = $db->prepare($sql);
                                    $stmt->execute([$name, $color, $id]);
                                }

                                // Funzione per l'eliminazione del gruppo
                                function deleteGroup($db, $id)
                                {
                                    $sql = "DELETE FROM dr_group WHERE id_group=?";
                                    $stmt = $db->prepare($sql);
                                    $stmt->execute([$id]);
                                }

                                // Verifica se è stata inviata una richiesta per aggiornare i dati del gruppo
                                if (isset($_POST['update_group'])) {
                                    $id = $_POST['id'];
                                    $name = $_POST['name'];
                                    $color = $_POST['color'];
                                    updateGroup($db, $id, $name, $color);
                                }

                                // Verifica se è stata inviata una richiesta per eliminare il gruppo
                                ...
                                // Query per ottenere i dati dei gruppi dalla tabella 'dr_group'
                                $sql = "SELECT * FROM dr_group";
                                $result = $db->query($sql);

                                // Verifica se sono presenti risultati
                                if ($result->rowCount() > 0) {
                                    // Output dei dati dei gruppi in una tabella
                                    ...
                                    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                                        ...
                                        echo "<td>" . $row["color_group"] . "</td>";
                                        // Form per l'aggiornamento dei dati
                                        echo "<td>";
                                        echo "<details>";
                                        echo "<summary>Update</summary>";
                                        echo "<form method='post' action=''>";
                                        echo "<input type='hidden' name='id' value='" . $row["id_group"] . "'>";
                                        echo "<input type='name' name='name' value='" . $row["name_group"] . "' required>";
                                        echo "<input type='color' name='color' value='" . $row["color_group"] . "' required>";
                                        echo "<input type='submit' name='update_group' value='Update' class='btn btn-info'>";
                                        echo "</form>";
                                        echo "</details>";
                                        echo "</td>";

                                        // Form per l'eliminazione del gruppo
                                        echo "<td>";
                                        echo "<form method='post' action=''>";
                                        echo "<input type='hidden' name='id' value='" . $row["id_group"] . "'>";
                                        echo "<input type='submit' name='delete_group' value='Delete' class='btn btn-danger' onclick='return confirm(\"Sei sicuro di voler eliminare questo gruppo?\")'>"; // Aggiungi la classe btn-danger qui
                                        echo "</form>";
                                        echo "</td>";
                                    }
                                    echo "</tbody>";
                                    ...
                                    echo "<p>Nessun gruppo trovato nel database.</p>";
                                }
                                ?>


                                <a href="creation_group.php" class="btn btn-primary">
