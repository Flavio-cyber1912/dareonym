...
$adminName = $_SESSION["user"][0]["email_admin"];
                ...
                <div class="card mb-4">
                    <div class="card-header border-bottom">
                        ...
                        <ul class="nav nav-tabs card-header-tabs" id="dashboardNav" role="tablist">
                            <li class="nav-item"><a class="nav-link active" id="activities-pill" href="#activities" data-bs-toggle="tab" role="tab" aria-controls="activities" aria-selected="true">Creators</a></li>
                        </ul>
                    </div>
                    <div class="card-body">
                        <div class="tab-content" id="dashboardNavContent">
                            ...
                            <div class="tab-pane fade active show" id="activities" role="tabpanel" aria-labelledby="activities-pill">
                                <?php
                                // Include il file di connessione al database
                                include_once("connection_database.php");

                                // Funzione per l'aggiornamento dell'obbligo
                                function updateObbligation($db, $id, $description_obbligation, $data_creation_obbligation)
                                {
                                    $sql = "UPDATE dr_obbligation SET description_obbligation=?, data_creation_obbligation=? WHERE id_obbligation=?";
                                    $stmt = $db->prepare($sql);
                                    $stmt->execute([$description_obbligation, $data_creation_obbligation, $id]);
                                }

                                // Funzione per l'eliminazione dell'obbligo
                                function deleteObbligation($db, $id)
                                {
                                    $sql = "DELETE FROM dr_obbligation WHERE id_obbligation=?";
                                    $stmt = $db->prepare($sql);
                                    $stmt->execute([$id]);
                                }
                                // Verifica se è stata inviata una richiesta per aggiornare i dati dell'obbligo
                                if (isset($_POST['update_obbligation'])) {
                                    $id = $_POST['id'];
                                    $description_obbligation = isset($_POST['description_obbligation']) ? $_POST['description_obbligation'] : null;
                                    $data_creation_obbligation = isset($_POST['data_creation_obbligation']) ? $_POST['data_creation_obbligation'] : null;
                                    updateObbligation($db, $id, $description_obbligation, $data_creation_obbligation);
                                }


                                // Verifica se è stata inviata una richiesta per eliminare l'obbligo
                                if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_obbligation'])) {
                                    $id = $_POST['id'];
                                    deleteObbligation($db, $id);
                                }

                                // Query per ottenere i dati degli obblighi dalla tabella 'dr_obbligation'
                                $sql = "SELECT * FROM dr_obbligation";
                                $result = $db->query($sql);

                                // Verifica se sono presenti risultati
                                if ($result->rowCount() > 0) {
                                    // Output dei dati degli obblighi in una tabella
                                    ...
                                    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                                        echo "<tr>";
                                        echo "<td>" . $row["id_obbligation"] . "</td>";
                                        echo "<td>" . $row["description_obbligation"] . "</td>";
                                        echo "<td>" . $row["data_creation_obbligation"] . "</td>";
                                        // Form per l'aggiornamento dei dati
                                        echo "<td>";
                                        echo "<details>";
                                        echo "<summary>Update</summary>";
                                        echo "<form method='post' action=''>";
                                        echo "<input type='hidden' name='id' value='" . $row["id_obbligation"] . "'>";
                                        echo "<input type='text' name='description_obbligation' value='" . $row["description_obbligation"] . "' required>";
                                        echo "<input type='date' name='data_creation_obbligation' value='" . $row["data_creation_obbligation"] . "' required>";
                                        echo "<input type='submit' name='update_obbligation' value='Update' class='btn btn-info'>";
                                        echo "</form>";
                                        echo "</details>";
                                        echo "</td>";
                                        // Form per l'eliminazione dell'obbligo
                                        echo "<td>";
                                        echo "<form method='post' action=''>";
                                        echo "<input type='hidden' name='id' value='" . $row["id_obbligation"] . "'>";
                                        echo "<input type='submit' name='delete_obbligation' value='Delete' class='btn btn-danger' onclick='return confirm(\"Sei sicuro di voler eliminare questo obbligo?\")'>";
                                        echo "</form>";
                                        echo "</td>";
                                        echo "</tr>";
                                    }
                                    ...
                                    echo "<p>Nessun obbligo trovato nel database.</p>";
                                }
                                ?>
